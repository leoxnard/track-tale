...
const en = {
  ...
  downloads: {
    ...
    trackNote:
      "The tracks come as they were recorded, at full resolution, rather than as the line the map is drawn from. The first track in the file is the whole trip merged into one long route; after it, each day is its own track, and a leg taken by train, ferry or bus is one of its own — so nothing counts kilometres nobody rode. A day uploaded before the recordings were kept comes as the map draws it.",
    /**
...
const de: Messages = {
  ...
  downloads: {
    ...
    trackNote:
      "Die Tracks kommen so, wie sie aufgezeichnet wurden, in voller Auflösung — nicht als die Linie, aus der die Karte gezeichnet wird. Der erste Track in der Datei ist die ganze Reise zu einer langen Tour zusammengefügt; danach ist jeder Tag ein eigener Track, und eine Etappe mit Zug, Fähre oder Bus noch einmal ein eigener — so zählt niemand Kilometer mit, die keiner gefahren ist. Ein Tag, der vor dem Aufheben der Aufzeichnungen hochgeladen wurde, kommt so, wie die Karte ihn zeichnet.",
    planNote:
